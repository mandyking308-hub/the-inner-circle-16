    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          ...
          {submitted ? (
            <div className="mt-7 border-t border-border pt-6">
              ...
              <p className="mt-3 text-sm leading-7 text-muted-foreground">
                Concierge will review the context first. If the introduction looks useful, we ask the other member privately before sharing anything.
              </p>
              <Button className="mt-5 rounded-none" variant="outline" onClick={() => setSubmitted(false)}>
            ...
            <form onSubmit={submit} className="mt-6 space-y-5">
              <div className="space-y-2">
                ...
                <Input id="person" required className="rounded-none" placeholder="Member name, role or type of person" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="reason">Why this person?</Label>
                <Textarea id="reason" required rows={4} className="rounded-none" placeholder="What makes the connection relevant now?" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="outcome">What would a useful outcome look like?</Label>
                <Textarea id="outcome" rows={3} className="rounded-none" placeholder="Advice, a specific conversation, learning, collaboration…" />
              </div>
              <Button type="submit" className="rounded-none">Send to concierge</Button>
            </form>
          )}
        </section>
        ...
        <section className="border border-border bg-card">
          ...
          <div>
            {statuses.map((item) => (
              <div key={item.person} className="grid gap-4 border-b border-border p-5 last:border-b-0 md:grid-cols-[1fr_auto] md:p-6">
                <div>
                  <p className="text-sm font-medium">{item.person}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{item.reason}</p>
                  <p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">{item.detail}</p>
                </div>
      ...
      <div className="flex items-start gap-3 border border-border bg-accent/25 p-4 text-xs leading-6 text-muted-foreground">
        ...
        Introductions are part of the community service, not inventory for sale. Members and advisers cannot pay to be placed in front of another member.
      </div>
    </div>
  );
}
