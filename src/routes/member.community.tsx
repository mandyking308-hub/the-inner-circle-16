...
import { ArrowRight, Languages, MapPin, Network, Search, ShieldCheck, Sparkles } from "lucide-react";
      ...
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
        {filtered.map(({ member, context }) => (
          <article key={member.id} className="border border-border bg-card p-5">
            ...
            {context ? <div className="mt-5 border-y border-border py-4"><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-oxblood">Why this person may be useful</p><p className="mt-2 text-xs leading-6">{context.whyRelevant}</p><div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-[10px] text-muted-foreground">{context.metAt ? <span className="inline-flex items-center gap-1.5"><Network className="h-3 w-3" />Met at {context.metAt}</span> : null}{context.otherCities.length ? <span className="inline-flex items-center gap-1.5"><MapPin className="h-3 w-3" />{context.otherCities.join(" · ")}</span> : null}{context.languages.length ? <span className="inline-flex items-center gap-1.5"><Languages className="h-3 w-3" />{context.languages.join(" · ")}</span> : null}</div>{context.mutualConnections.length ? <p className="mt-3 text-[10px] text-muted-foreground">Mutual context: {context.mutualConnections.join(", ")}</p> : null}</div> : null}
            ...
            <Link to="/member/introductions" search={{ person: member.name } as never} className="mt-6 inline-flex items-center gap-2 text-sm font-semibold">Request a warm introduction <ArrowRight className="h-3.5 w-3.5" /></Link>
          </article>
        ))}
      </div>
