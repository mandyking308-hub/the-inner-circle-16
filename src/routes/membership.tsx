  ...
  return (
    <>
      ...
      <section className="relative min-h-[78vh] overflow-hidden bg-[#11110f] text-white">
        ...
        <Container className="relative flex min-h-[78vh] items-end py-20 md:py-28">
          <div className="max-w-3xl">
            <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#d8b36c]">By invitation · Founding membership {membershipPricing.pricingYear}</p>
            <h1 className="mt-6 max-w-[13ch] font-display text-5xl leading-[0.96] md:text-7xl">A seat at a very private table.</h1>
            <p className="mt-7 max-w-xl text-base leading-8 text-white/75">
              Montvelle is a small, deliberately quiet membership for families and principals building something
              intended to outlast them. Fewer people. Longer conversations. Nothing said twice.
            </p>
            <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-4">
              <a href="#join" className="inline-flex items-center gap-2 border-b border-[#d8b36c] pb-1 text-sm font-medium tracking-wide text-[#d8b36c] transition-colors hover:text-[#e8c98d]">
                Membership & enrolment <ArrowRight className="h-4 w-4" />
              </a>
              <Link to="/apply" className="text-sm text-white/60 underline-offset-4 transition-colors hover:text-white/90 hover:underline">
      ...
      <section className="bg-background py-24 md:py-32">
        <Container>
          <div className="max-w-3xl">
            <p className="text-[10px] font-semibold uppercase tracking-[0.26em] text-oxblood">The invitation</p>
            ...
            <p className="mt-8 text-base leading-8 text-muted-foreground md:text-lg">
              What follows is the whole of it — the people, the places, the family years, the private service and the
              office behind it. Held together as one membership, so that nothing about your life has to be explained
              more than once.
            </p>
          </div>
          ...
          <div className="mt-16 grid gap-px border border-foreground/12 bg-foreground/12 md:grid-cols-3">
            {invitation.map((item) => (
              <div key={item.label} className="bg-background p-8 md:p-10">
                <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-oxblood">{item.label}</p>
      ...
      <section className="bg-[#f4ede1] py-24 md:py-32">
        <Container>
          <div className="grid gap-14 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.26em] text-oxblood">What membership holds</p>
              <h2 className="mt-6 font-display text-4xl leading-[1.04] md:text-5xl">One membership, quietly complete.</h2>
            </div>
            <div>
              {included.map((item) => (
                <div key={item.title} className="border-t border-foreground/15 py-7 first:border-t-0 first:pt-0">
      ...
      <section className="py-24 md:py-32">
        <Container>
          <div className="grid gap-14 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-24">
            ...
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.26em] text-oxblood">Selective, not complicated</p>
              <h2 className="mt-5 font-display text-4xl leading-[1.04] md:text-6xl">The price is public. The standard remains private.</h2>
              <p className="mt-7 text-base leading-8 text-muted-foreground">
                A fee opens a conversation; it does not purchase anyone else's confidence. Montvelle remains a curated
                membership, with strict confidentiality, consent-led introductions, and the quiet freedom to protect the
                community whenever conduct, safety, law or trust asks it of us.
              </p>
            </div>
          </div>
        </Container>
            ...
            <div>
              <div className="border border-foreground/12 bg-[#171716] p-8 text-white md:p-12">
                ...
                <p className="mt-6 max-w-md text-sm leading-7 text-white/60">
                  ...
                  sees or stores your card or bank credentials.
                </p>

                <form onSubmit={beginCheckout} className="mt-10 space-y-7">
                  <div className="space-y-2.5">
                    <Label htmlFor="membership-name" className="text-[10px] font-semibold uppercase tracking-[0.2em] text-white/50">Full name</Label>
                    <Input id="membership-name" name="name" required autoComplete="name" className="h-12 rounded-none border-0 border-b border-white/20 bg-transparent px-0 text-white shadow-none focus-visible:border-[#d8b36c] focus-visible:ring-0" />
                  </div>
                  <div className="space-y-2.5">
                    <Label htmlFor="membership-email" className="text-[10px] font-semibold uppercase tracking-[0.2em] text-white/50">Email</Label>
                    <Input id="membership-email" name="email" type="email" required autoComplete="email" className="h-12 rounded-none border-0 border-b border-white/20 bg-transparent px-0 text-white shadow-none focus-visible:border-[#d8b36c] focus-visible:ring-0" />
                  </div>
                  <div className="space-y-2.5">
                    <Label htmlFor="membership-country" className="text-[10px] font-semibold uppercase tracking-[0.2em] text-white/50">Country of residence</Label>
                    <Input id="membership-country" name="country" required autoComplete="country-name" className="h-12 rounded-none border-0 border-b border-white/20 bg-transparent px-0 text-white shadow-none focus-visible:border-[#d8b36c] focus-visible:ring-0" />
                  </div>

                  <div className="space-y-5 border-t border-white/12 pt-8">
                    <label className="flex gap-4 text-[11px] leading-6 text-white/55">
                      <input type="checkbox" required className="mt-1 h-3.5 w-3.5 shrink-0 accent-[#d8b36c]" />
                      <span>
                        I have read and agree to the <Link to="/membership-agreement" className="text-white/80 underline underline-offset-2">Membership Agreement</Link>,{" "}
                        <Link to="/terms" className="text-white/80 underline underline-offset-2">Website Terms</Link>,{" "}
                        <Link to="/privacy" className="text-white/80 underline underline-offset-2">Privacy Notice</Link> and{" "}
                        <Link to="/confidentiality" className="text-white/80 underline underline-offset-2">Confidentiality &amp; No Solicitation standard</Link>. My Membership Schedule records the approved member, term and commercial details.
                      </span>
                    </label>
                    <label className="flex gap-4 text-[11px] leading-6 text-white/55">
                      <input type="checkbox" required className="mt-1 h-3.5 w-3.5 shrink-0 accent-[#d8b36c]" />
                      <span>
                        I ask that onboarding and service preparation begin as soon as GSM accepts my membership. Where a
                        statutory cancellation right applies, services already supplied may be deducted from any refund to
                    ...
                    <label className="flex gap-4 text-[11px] leading-6 text-white/55">
                      <input type="checkbox" required className="mt-1 h-3.5 w-3.5 shrink-0 accent-[#d8b36c]" />
                      <span>
                        I understand that payment does not override GSM's admission, sanctions, fraud, safety or compliance
                        checks. If a membership is not accepted, the membership and admission fees paid for it are returned,
                  ...
                  {error ? (
                    <p className="border border-[#d8b36c]/35 bg-[#d8b36c]/8 p-4 text-xs leading-6 text-[#f1d69d]" role="alert">{error}</p>
                  ) : null}

                  <Button type="submit" size="lg" disabled={submitting} className="h-14 w-full rounded-none bg-[#d8b36c] text-sm font-semibold tracking-wide text-[#15130f] hover:bg-[#e4c47f]">
                    {submitting ? "Opening secure checkout…" : `Continue securely · ${membershipPricing.firstYearDisplay}`}
                ...
                <div className="mt-10 flex gap-4 border-t border-white/12 pt-7">
                  ...
                  <p className="text-[10px] leading-5 text-white/45">
                    Montvelle is operated by {site.operator}, a Delaware limited liability company. Membership activates
                    only after acceptance, required checks, contractual acceptance and cleared funds.
                  </p>
                </div>
              </div>
              ...
              <div className="mt-8 flex flex-wrap items-end justify-between gap-6 border-t border-foreground/15 pt-8">
                <div>
                  ...
                  <p className="mt-3 max-w-md text-sm leading-7 text-muted-foreground">
                    Many families would rather sit with us before anything is signed. That door is always open.
                  </p>
                </div>
                <Link to="/apply" className="inline-flex items-center gap-2 border-b border-oxblood pb-1 text-sm font-medium text-oxblood transition-opacity hover:opacity-70">
