    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.3fr_0.7fr]">
        <section className="border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border p-5 md:p-6"><div><p className="eyebrow text-bronze">Active case room</p><h2 className="mt-2 font-display text-3xl">International family move · early planning</h2></div></div>
          <div className="grid gap-px bg-border sm:grid-cols-2 lg:grid-cols-3">
            {globalLifeWorkstreams.map((workstream) => (
              <article key={workstream.name} className="bg-background p-5">
                <div className="flex items-center justify-between gap-3"><h3 className="font-display text-2xl">{workstream.name}</h3></div>
        ...
        <section className="border border-border bg-foreground p-6 text-background">
          ...
          <Button asChild variant="outline" className="mt-7 w-full rounded-none border-background/30 bg-transparent text-background hover:bg-background hover:text-foreground"><Link to="/member/concierge">Ask concierge to coordinate </Link></Button>
        </section>
      </div>
      ...
      <section>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {["Residence rights and renewal", "Tax residence and exit/entry timing", "Companies, trusts and ownership", "Banking, FX and payment rails", "Property and household logistics", "Schools and curriculum continuity", "Healthcare and insurance", "Wills, succession and incapacity", "Travel-day calendar and key dates"].map((item) => <div key={item} className="flex items-start gap-3 border border-border bg-card p-4 text-sm"><span>{item}</span></div>)}
        </div>
      </section>
