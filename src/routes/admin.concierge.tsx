...
import { Clock3, Compass, UserRoundCheck } from "lucide-react";
...
function AdminConciergePage() {
  return (
    <div className="space-y-8">
      ...
      <section className="border border-border bg-card"><div className="border-b border-border p-5"><div className="flex items-center gap-3"><Compass className="h-5 w-5 text-bronze" /><h2 className="font-display text-3xl">Request queue</h2></div></div><div className="divide-y divide-border">{conciergeRequests.map((request) => <article key={request.id} className="grid gap-4 p-5 md:grid-cols-[1fr_140px_150px_210px_auto] md:items-center"><div><p className="text-[10px] uppercase tracking-[0.15em] text-bronze">{request.id} · {request.category}</p><h3 className="mt-1 font-display text-2xl">{request.title}</h3><p className="mt-2 text-xs text-muted-foreground">Member: {request.member}</p></div><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Owner</p><p className="mt-1 text-sm">{request.owner}</p></div><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Status</p><p className="mt-1 text-sm">{request.status}</p></div><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Next action</p><p className="mt-1 text-xs leading-5">{request.nextStep}</p></div><Button variant="outline" className="rounded-none">Open</Button></article>)}</div></section>
