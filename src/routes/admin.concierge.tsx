...
import { readSourcingCases, writeSourcingCases, type SourcingCase } from "@/data/sourcing";
...
function AdminConciergePage() {
  const [sourcing, setSourcing] = useState<SourcingCase[]>([]);
  useEffect(() => { setSourcing(readSourcingCases()); }, []);

  const needsSourcing = (caseId: string) => sourcing.some((item) => item.caseId === caseId);

  const sendToSourcing = (request: (typeof conciergeRequests)[number]) => {
    if (needsSourcing(request.id)) return;
    const next: SourcingCase = {
      ...
      benchReviewed: false,
      consent: "neutral",
      memberContext: request.nextStep,
      neutralBrief: `A private client office is seeking ${request.title.toLowerCase()}. Discretion is expected. No client details will be shared at this stage.`,
      shortlist: [],
    };
    const updated = [next, ...sourcing];
    setSourcing(updated);
    writeSourcingCases(updated);
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="border border-border bg-card"><div className="border-b border-border p-5"><div className="flex items-center gap-3"><Compass className="h-5 w-5 text-bronze" /><h2 className="font-display text-3xl">Request queue</h2></div></div><div className="divide-y divide-border">{conciergeRequests.map((request) => <article key={request.id} className="grid gap-4 p-5 md:grid-cols-[1fr_140px_150px_210px_auto] md:items-center"><div><p className="text-[10px] uppercase tracking-[0.15em] text-bronze">{request.id} · {request.category}</p><h3 className="mt-1 font-display text-2xl">{request.title}</h3><p className="mt-2 text-xs text-muted-foreground">Member: {request.member}</p></div><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Owner</p><p className="mt-1 text-sm">{request.owner}</p></div><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Status</p><p className="mt-1 text-sm">{request.status}</p></div><div><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Next action</p><p className="mt-1 text-xs leading-5">{request.nextStep}</p></div><div className="flex flex-wrap gap-2">{needsSourcing(request.id) ? <Button asChild variant="outline" className="rounded-none"><Link to="/admin/sourcing"><FileSearch className="mr-2 h-4 w-4" />In sourcing</Link></Button> : <Button variant="outline" className="rounded-none" onClick={() => sendToSourcing(request)}>Needs sourcing</Button>}<Button variant="ghost" className="rounded-none">Open</Button></div></article>)}</div></section>
      <div className="grid gap-4 lg:grid-cols-2"><section className="border border-border bg-card p-6"><Clock3 className="h-5 w-5 text-bronze" /><h2 className="mt-5 font-display text-3xl">Service standard</h2><p className="mt-4 text-sm leading-7 text-muted-foreground">Acknowledge quickly, clarify before searching, assign one owner, never leave a request without a next action, and close the loop explicitly when the member agrees the job is done.</p></section><section className="border border-border bg-card p-6"><UserRoundCheck className="h-5 w-5 text-bronze" /><h2 className="mt-5 font-display text-3xl">Consent standard</h2><p className="mt-4 text-sm leading-7 text-muted-foreground">No partner receives more member context than necessary before consent. Sensitive details stay inside the member relationship until the member chooses to instruct or connect.</p></section></div>
    </div>
  );
}
