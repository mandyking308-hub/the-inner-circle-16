  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-5 xl:grid-cols-[0.72fr_1.28fr]">
        ...
        <div className="border border-border bg-card p-6">{selected ? <><div className="flex flex-wrap items-start justify-between gap-5"><div><p className="eyebrow text-oxblood">{selected.id}</p><h2 className="mt-2 font-display text-4xl">{selected.firm}</h2><p className="mt-2 text-sm text-muted-foreground">{selected.contactName} · {selected.email}</p></div><select value={selected.status} onChange={(event) => setApplications((current) => current.map((application) => application.id === selected.id ? { ...application, status: event.target.value as PartnerApplicationStatus } : application))} className="h-9 rounded-none border border-input bg-background px-3 text-xs">{statuses.map((status) => <option key={status}>{status}</option>)}</select></div><div className="mt-6 grid gap-4 sm:grid-cols-2">{[["Regulatory / professional status", selected.regulatoryStatus],["Relevant family experience", selected.familyExperience],["Why members need them", selected.whyRelevant],["Member benefit", selected.memberBenefit || "None proposed"],["Conflicts / commercial disclosure", selected.conflicts || "None supplied"]].map(([label, value]) => <div key={label} className="border-t border-border pt-4"><p className="text-[9px] uppercase tracking-[0.14em] text-oxblood">{label}</p><p className="mt-2 text-xs leading-6 text-muted-foreground">{value}</p></div>)}</div><div className="mt-7"><ReferencePanel references={selected.references} /><div className="mt-4 flex flex-wrap items-center gap-2"><p className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Reference check</p><Button type="button" variant={selected.status === "References" ? "default" : "outline"} className="rounded-none" onClick={() => setApplications((current) => current.map((application) => application.id === selected.id ? { ...application, status: "References" } : application))}>References to check</Button><Button type="button" variant={selected.status === "Conversation" ? "default" : "outline"} className="rounded-none" onClick={() => setApplications((current) => current.map((application) => application.id === selected.id ? { ...application, status: "Conversation" } : application))}>References checked</Button></div></div><div className="mt-7 border-t border-border pt-5"><p className="eyebrow text-oxblood">Screening questions</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{partnerQualification.map((item) => <div key={item.key} className="border border-border bg-background p-4"><p className="text-sm font-semibold">{item.label}</p><p className="mt-2 text-xs leading-6 text-muted-foreground">{item.question}</p></div>)}</div></div></> : <p className="text-sm text-muted-foreground">Select a partner application.</p>}</div>
      </section>
