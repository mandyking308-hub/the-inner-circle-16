  ...
  head: () => ({
    meta: [
      { title: `The Ecosystem — ${site.name}` },
      {
        name: "description",
        content:
          "How Montvelle connects trusted peers, a Life Decision Room, global life coordination, family learning, sourced outside expertise, concierge execution and a rising-generation network.",
      },
    ],
  }),
      ...
      <section className="border-b border-border py-20 md:py-28">
        <Container>
          ...
          <p className="mt-7 max-w-3xl text-base leading-8 text-muted-foreground">
            A private community becomes much more useful when it can convert collective judgement
            into clear decisions, trusted expertise, coordinated action and capability across
            generations. Each layer below solves a different part of that problem.
          </p>
        </Container>
      </section>
            ...
            {layers.map((item, index) => {
              ...
              return (
                <Link
                  key={item.title}
                  to={item.to}
                  className="group bg-background p-7 transition-colors hover:bg-card md:p-8"
                >
      ...
      <section className="border-b border-border bg-foreground py-16 text-background md:py-24">
        <Container>
          <div className="grid gap-10 lg:grid-cols-[0.75fr_1.25fr]">
            <div>
              <p className="eyebrow text-background/60">The execution principle</p>
              <h2 className="mt-4 font-display text-5xl leading-[1.02]">
                Ask → decide → coordinate → do → learn.
              </h2>
            </div>
            <div className="grid gap-px bg-background/20 sm:grid-cols-5">
              {["Ask", "Decide", "Coordinate", "Do", "Learn"].map((step, index) => (
                <div key={step} className="bg-foreground p-5">
                  <p className="font-display text-3xl text-bronze">0{index + 1}</p>
                  <p className="mt-8 text-sm uppercase tracking-[0.15em] text-background/75">
                    {step}
                  </p>
                </div>
              ))}
