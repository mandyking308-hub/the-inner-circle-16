        ...
        request.id === requestId
          ? {
              ...
              options: request.options.map((option) =>
                option.id === optionId
                  ? { ...option, status: "Chosen" as const }
                  : { ...option, status: "Set aside" as const },
              ),
              ...
              updates: [
                ...request.updates,
                {
                  ...
                  note: "You chose an option. We will arrange it and confirm.",
                },
              ],
            }
          : request,
          ...
          <div className="flex items-start gap-3">
            <div>
              ...
              <h2 className="mt-3 font-display text-3xl leading-tight">
                We have this. We'll come back to you within 24 hours.
              </h2>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-background/65">
                Someone will be with it today. Where a request is complex, the 24 hours is our
                promise of a meaningful reply — not a claim that everything can be solved by then.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="grid gap-px border border-border bg-border md:grid-cols-3">
        {[
          ...
          [
            ...
            "We go out and make the enquiries ourselves, with the minimum anyone needs to know.",
          ],
          [
            "Within 24 hours",
            "An answer, a progress update, or checked options — always something real.",
          ],
        ].map(([title, note]) => (
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            name="city"
            className="rounded-none"
            placeholder="City or country, if it applies"
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            name="timeframe"
            className="rounded-none"
            placeholder="Fixed dates, or a window"
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            name="logistics"
            className="rounded-none"
            placeholder="Numbers, household, luggage, access"
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            name="budget"
            className="rounded-none"
            placeholder="Leave blank if you would rather see the market"
          />
        </div>
        <div className="space-y-2 md:col-span-2">
          ...
          <Textarea
            ...
            rows={3}
            className="rounded-none"
            placeholder="Standards, dislikes, discretion requirements, anyone already involved."
          />
        </div>
        ...
        <label className="flex items-start gap-3 border-t border-border pt-4 text-xs leading-6 md:col-span-2">
          ...
          <span>
            <strong>Please handle this for me.</strong> Montvelle should make the enquiries, arrange
            it and stay with it through to the day, rather than returning options for me to organise
            myself.
          </span>
        </label>
        ...
        <div className="flex items-start gap-3">
          <div>
            ...
            <p className="mt-2 max-w-4xl text-xs leading-6 text-muted-foreground">
              We approach people on your behalf with only what they need in order to answer — the
              shape of the request, the place and the timing. Your name, your household and the
              reason behind the request stay with us unless you tell us otherwise.
            </p>
            <p className="mt-3 max-w-4xl text-xs leading-6 text-muted-foreground">
              A request you make is private to you by default. Sharing one with another approved
              member of your household is a deliberate choice, matter by matter — you can ask us to
              open it up at any point.
            </p>
          </div>
        </div>
      </section>
            ...
            {requests.map((request) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <span
                    className={`text-[9px] uppercase tracking-[0.14em] ${selected?.id === request.id ? "text-bronze" : "text-oxblood"}`}
                  >
                    {request.status}
                  </span>
                ...
                <p
                  className={`mt-2 text-[11px] leading-5 ${selected?.id === request.id ? "text-background/60" : "text-muted-foreground"}`}
                >
                  {request.respondedAt
                    ? request.nextUpdate
                    : "Next update: within 24 hours of receipt"}
                </p>
              </button>
            ))}
            {requests.length === 0 ? (
              <p className="p-5 text-sm text-muted-foreground">Nothing open yet.</p>
            ) : null}
          </div>
        </aside>
            ...
            <article className="border border-border bg-foreground p-6 text-background md:p-8">
              ...
              <dl className="mt-7 grid gap-4 border-t border-background/15 pt-6 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  <div key={label}>
                    <dt className="text-[9px] uppercase tracking-[0.14em] text-background/45">
                      {label}
                    </dt>
            ...
            <article className="border border-border bg-card p-6">
              ...
              <ol className="mt-6 grid gap-px bg-border sm:grid-cols-3 lg:grid-cols-6">
                {memberRequestStatuses.map((status) => {
                  const reached =
                    memberRequestStatuses.indexOf(status) <=
                    memberRequestStatuses.indexOf(selected.status);
                  return (
                    <li key={status} className={`bg-card p-4 ${reached ? "" : "opacity-45"}`}>
                      <p
                        className={`text-[9px] uppercase tracking-[0.13em] ${reached ? "text-oxblood" : "text-muted-foreground"}`}
                      >
                        {status}
                      </p>
                    </li>
              ...
              <p className="mt-5 text-sm leading-7 text-muted-foreground">
                {memberStatusNote[selected.status]}
              </p>
              <p className="mt-2 text-sm leading-7">
                <span className="text-[10px] uppercase tracking-[0.14em] text-oxblood">
                  Next update ·{" "}
                </span>
              ...
              <div className="mt-6 space-y-3 border-t border-border pt-5">
                {selected.updates.map((update) => (
                  <div key={update.id} className="flex gap-4">
                    <p className="w-24 shrink-0 text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
                      {update.at}
                    </p>
            ...
            <article className="border border-border bg-card p-6">
              ...
              {selected.options.length ? (
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        ...
                        {option.status === "Chosen" ? (
                          ...
                          <Button
                            variant="outline"
                            className="rounded-none"
                            onClick={() => chooseOption(selected.id, option.id)}
                          >
                            Take this one
                          </Button>
                        )}
                ...
                <p className="mt-4 max-w-3xl text-sm leading-7 text-muted-foreground">
                  Nothing here yet. Options appear only once we have spoken to whoever is behind
                  them and checked what they said.
                </p>
              )}
            </article>
          </section>
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="eyebrow text-background/50">
              Something larger than a single arrangement?
            </p>
            <h2 className="mt-3 font-display text-3xl">
              Give the whole problem to Private Office.
            </h2>
          </div>
          <Button
            asChild
