...
const memberGroups = [
  ...
  },
  {
    label: "Programme",
    items: [
      { to: "/member/events", label: "Gatherings", icon: CalendarDays },
...
export function PrivateShell({ mode, children }: PrivateShellProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 border-b border-foreground/15 bg-background/95 backdrop-blur-xl lg:pl-[270px]">
        <div className="flex h-[68px] items-center justify-between px-4 md:px-7 lg:px-9">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-oxblood">{mode === "member" ? "Private member workspace" : "Concierge & operations"}</p>
              <p className="mt-1 font-display text-xl text-foreground">{mode === "member" ? "Your family enterprise" : "The house operating desk"}</p>
            </div>
          </div>
      ...
      <aside className={`${mobileOpen ? "fixed inset-x-0 top-[68px] z-50 block h-[calc(100vh-68px)]" : "hidden"} bg-foreground text-background lg:fixed lg:inset-y-0 lg:left-0 lg:z-50 lg:block lg:w-[270px]`}>
        <div className="flex h-full flex-col">
          <Link to="/" className="flex h-[96px] items-center gap-3 border-b border-background/12 px-5">
            <BrandMark inverse />
            <div><p className="font-display text-2xl leading-none">{site.name}</p><p className="mt-2 text-[8px] font-semibold uppercase tracking-[0.2em] text-background/40">Private operating system</p></div>
          </Link>
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={`group flex items-center justify-between border-l-2 px-3 py-2.5 text-[12px] transition-colors ${
                          active
                            ? "border-oxblood bg-background/8 text-background"
                            : "border-transparent text-background/55 hover:bg-background/5 hover:text-background"
                        }`}
                      >
                        <span className="flex items-center gap-3"><Icon className={`h-3.5 w-3.5 ${active ? "text-bronze" : "text-background/35"}`} />{item.label}</span>
          ...
          <div className="border-t border-background/12 p-5">
            <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-bronze" /><div><p className="text-[9px] font-semibold uppercase tracking-[0.17em] text-background/60">Confidential by default</p><p className="mt-2 text-[10px] leading-5 text-background/40">No member data export, cold solicitation or private-room content harvesting.</p></div></div>
          </div>
        </div>
      </aside>

      <main className="min-w-0 px-4 py-7 md:px-8 md:py-9 lg:ml-[270px] lg:px-10 xl:px-12">{children}</main>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-5 border-b border-foreground/15 pb-7 md:flex-row md:items-end md:justify-between">
      <div className="max-w-4xl">
        ...
        <h1 className="mt-3 text-balance font-display text-4xl leading-[0.98] text-foreground md:text-5xl xl:text-6xl">{title}</h1>
