...
export function LanguageSelector({ inverse = false, compact = false }: { inverse?: boolean; compact?: boolean }) {
  ...
  const id = compact ? "montvelle-language-mobile" : inverse ? "montvelle-language-footer" : "montvelle-language";
  ...
  return (
    <div className={`group relative inline-flex items-center ${inverse ? "text-white/72" : "text-muted-foreground"}`}>
      ...
      <label htmlFor={id} className="sr-only">Choose language</label>
      <select
        id={id}
        defaultValue="en"
        onChange={handleChange}
        aria-label="Translate Montvelle"
        className={`appearance-none rounded-full border bg-transparent py-2 pl-8 pr-7 text-[10px] font-medium tracking-[0.03em] outline-none transition-colors ${inverse ? "border-white/18 text-white/78 hover:border-white/32 focus:border-white/48" : "border-foreground/12 text-foreground/68 hover:border-foreground/25 focus:border-foreground/35"}`}
      >
