...
type CookiePreferences = { necessary: true; functional: boolean; analytics: boolean; other: boolean };
...
export function CookieBanner() {
  ...
  useEffect(() => {
    ...
    else { applyConsent(DEFAULTS); setShowBanner(true); }

    const openSettings = () => { setPrefs(getStoredPrefs() ?? DEFAULTS); setShowManage(true); };
  ...
  return (
    <>
      {showBanner && <div role="dialog" aria-label="Cookie choices" aria-live="polite" className="fixed inset-x-0 bottom-0 z-[80] border-t border-white/12 bg-[#171716] text-white shadow-2xl"><div className="mx-auto flex max-w-[92rem] flex-col gap-5 px-6 py-5 sm:px-8 md:flex-row md:items-center md:justify-between md:px-12 lg:px-16 xl:px-20"><div className="max-w-3xl"><p className="font-display text-xl">Your privacy choices</p><p className="mt-1 text-xs leading-6 text-white/58">Necessary storage keeps Montvelle secure and working. Optional functional and analytics technologies stay off unless you choose them. Read our <Link to="/cookies" className="underline underline-offset-4">Cookie Notice</Link>.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" className="border-white/25 bg-transparent text-white hover:bg-white hover:text-foreground" onClick={() => persist(DEFAULTS)}>Reject non-essential</Button><Button variant="outline" className="border-white/25 bg-transparent text-white hover:bg-white hover:text-foreground" onClick={() => setShowManage(true)}>Manage</Button><Button className="bg-white text-foreground hover:bg-white/90" onClick={() => persist({ necessary: true, functional: true, analytics: true, other: true })}>Accept all</Button></div></div></div>}

      <Dialog open={showManage} onOpenChange={setShowManage}>
        <DialogContent className="max-w-xl bg-[#f6f1e8]">
          <DialogHeader><DialogTitle className="font-display text-3xl">Manage privacy choices</DialogTitle><DialogDescription>Strictly necessary storage is always active. Optional categories are off unless you enable them.</DialogDescription></DialogHeader>
          ...
          <div className="flex flex-wrap justify-end gap-2 border-t border-foreground/10 pt-5"><Button variant="outline" onClick={() => persist(DEFAULTS)}>Reject non-essential</Button><Button variant="outline" onClick={() => persist({ necessary: true, functional: true, analytics: true, other: true })}>Accept all</Button><Button onClick={() => persist(prefs)}>Save choices</Button></div>
        </DialogContent>
      </Dialog>
    </>
  );
